...
import { QmsDocumentService } from './qms-document.service';
import { QmsDocument, QmsDocumentType } from './qms.models';
...
@Component({
  ...
  template: `
    <div class="page-head"><div><span class="eyebrow">QUALITY MANAGEMENT SYSTEM</span><h1>Document Control Dashboard</h1><p>Product specifications, engineering changes and controlled acceptance standards.</p></div><a mat-flat-button color="primary" routerLink="/qms/documents/new"><mat-icon>add</mat-icon>New document</a></div>
    <section class="metric-grid">
      @for (card of cards; track card) {
        <a class="metric" [routerLink]="['/qms/documents']" [queryParams]="{type:card.type}"><span class="metric-icon" [class]="card.tone"><mat-icon>{{ card.icon }}</mat-icon></span><span><strong>{{ count(card.type) }}</strong><small>{{ card.label }}</small></span><mat-icon class="go">arrow_forward</mat-icon></a>
      }
    </section>
    <section class="dashboard-grid">
      <article class="panel activity"><div class="panel-head"><div><h2>Recent activities</h2><p>Latest controlled document updates</p></div><a mat-button routerLink="/qms/documents">View all</a></div><app-qms-document-table [documents]="documents().slice(0, 5)" /></article>
      ...
      <a [routerLink]="['/qms/documents/new']" [queryParams]="{type:item.type}"><mat-icon>{{ item.icon }}</mat-icon><span>{{ item.label }}</span><mat-icon>chevron_right</mat-icon></a>
    }</div></article>
    </section>`,
  styleUrl: './qms.scss', changeDetection: ChangeDetectionStrategy.OnPush,
})
export class QmsDashboardComponent implements OnInit {
  private readonly service = inject(QmsDocumentService);
