...
import { ApprovalStep, QmsDocumentDefinition, QmsDocumentType, QmsFieldDefinition, QMS_DOCUMENT_DEFINITIONS, definitionFor } from './qms.models';
...
@Component({
  ...
  template: `
    <div class="page-head compact"><div><a class="back" routerLink="/qms/documents"><mat-icon>arrow_back</mat-icon>Document register</a><span class="eyebrow">{{ definition().formNumber }}</span><h1>{{ documentId ? 'Edit' : 'Create' }} {{ definition().name.en }}</h1><p>{{ definition().name.vi }} · {{ definition().name.zh }}</p></div><span class="draft-pill">Draft</span></div>
...
export class QmsDocumentEditorComponent implements OnInit {
  private readonly service = inject(QmsDocumentService); private readonly route = inject(ActivatedRoute); private readonly router = inject(Router); private readonly snack = inject(MatSnackBar);
