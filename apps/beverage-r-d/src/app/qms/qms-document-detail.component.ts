...
import { QmsDocument, QmsFieldDefinition, QmsStatus, definitionFor } from './qms.models';
...
@Component({
  ...
  template: `
    @if (document(); as item) {
      <div class="page-head compact"><div><a class="back" routerLink="/qms/documents"><mat-icon>arrow_back</mat-icon>Document register</a><span class="eyebrow">{{ item.documentNumber }}</span><h1>{{ definition(item).name.en }}</h1><p>{{ item.productCode }} · {{ item.productName }}</p></div><div class="detail-actions"><span class="status-large" [attr.data-status]="item.status">{{ statusName(item.status) }}</span><button mat-stroked-button (click)="print()"><mat-icon>picture_as_pdf</mat-icon>Print / PDF</button><a mat-flat-button color="primary" [routerLink]="['/qms/documents', item.id, 'edit']"><mat-icon>edit</mat-icon>Edit</a></div></div>
...
export class QmsDocumentDetailComponent implements OnInit {
  private readonly service = inject(QmsDocumentService); private readonly route = inject(ActivatedRoute); private readonly router = inject(Router); private readonly dialog = inject(MatDialog); readonly document = signal<QmsDocument | null>(null);
  ...
  remove(item: QmsDocument): void { this.dialog.open(QmsConfirmDialogComponent, { data: item, width: '430px' }).afterClosed().pipe(switchMap((confirmed) => confirmed ? this.service.delete(item.id) : [])).subscribe(() => this.router.navigate(['/qms/documents'])); }
}
