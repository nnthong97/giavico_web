...
import { QmsDocument, QmsDocumentType, QmsStatus, QMS_DOCUMENT_DEFINITIONS, QMS_STATUSES } from './qms.models';
...
@Component({
  ...
  template: `
    <div class="page-head"><div><span class="eyebrow">CONTROLLED RECORDS</span><h1>Product Specification & Change Control</h1><p>Search, review and maintain the complete QMS document register.</p></div><a mat-flat-button color="primary" routerLink="/qms/documents/new"><mat-icon>add</mat-icon>Create document</a></div>
...
export class QmsDocumentListPageComponent implements OnInit {
  private readonly service = inject(QmsDocumentService); private readonly dialog = inject(MatDialog); private readonly route = inject(ActivatedRoute);
